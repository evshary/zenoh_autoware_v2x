...
class Intersection:
    ...
    def _distance_measure(self, vehicle_id, vehicle_pos, lane_id):
        ...
        if distance <= 20:
            ...
            logging.debug(
                f"[Traffic Manager] Vehicle {vehicle_id} added to lane {lane_id}'s heap with distance {distance}"
            )
    ...
    async def monitor_request_queue(self, tx):
        while True:
            ...
            if has_requests:
                logging.debug(
                    "[Traffic Manager] monitor_request_queue", self.request_queue
                )
...
async def _send_decision(session, tx):
    while True:
        while not tx.empty():
            ...
            logging.info(
                f"[Traffic Manager] Sending Query '{get_key}'...payload: '{payload}'..."
            )
